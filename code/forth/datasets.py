...
class TrafficDataset:
    ...
    def construct_set(self, train_por = 0.6, test_por = 0.2, window_size = 12, label=0):
        ...
        # 补全构造过程
        #训练集
        len_train = int(self.raw_data.shape[0] * train_por)
        ...
        #验证集
        len_val = int(self.raw_data.shape[0] * (train_por+test_por))
        val_seqs = self.raw_data[len_train:len_val]
        for i in range(val_seqs.shape[0] - window_size):
        ...
        #测试集
        test_seqs = self.raw_data[len_val:]
        for i in range(test_seqs.shape[0] - window_size):
